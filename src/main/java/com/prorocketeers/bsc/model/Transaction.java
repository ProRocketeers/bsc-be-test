...
public class Transaction {
  ...
  public static void validateInput(@NotNull String line) throws TransactionParseException {
    ...
    if (data.length != EXACT_LINE_WORD_COUNT) {
      ...
      throw new TransactionParseException(
          "Currency is specified in wrong format, must only uppercase letters. "
              + data[CODE_INDEX]);
    } else if (!onlyDigitsPattern.matcher(data[AMOUNT_INDEX]).matches()) {
      throw new TransactionParseException(
          "Amount must be specified only as float number. " + data[AMOUNT_INDEX]);
    }
  }
  ...
  Transaction fromInput(@NotNull String line) {
    val data = line.split(" ");
  ...
  public String toOutput(@Nullable BigDecimal exchangeRate, @Nullable String exchangeCode) {
    val totalAmount = this.amount.setScale(0, RoundingMode.HALF_UP);
    val usdAmount = exchangeRate != null
        ? " (" + (this.amount.multiply(exchangeRate).setScale(2, RoundingMode.HALF_UP)) + " "
        + exchangeCode + ")"
        : "";
    return this.currencyCode + " " + totalAmount + usdAmount;
  }
