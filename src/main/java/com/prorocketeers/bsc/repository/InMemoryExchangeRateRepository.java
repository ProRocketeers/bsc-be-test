...
public class InMemoryExchangeRateRepository implements ExchangeRateRepository {
   ...
   */
  @Override
  public Optional<BigDecimal> get(@NotNull String from, @NotNull String to) {
    val rate = data.get(from + "-" + to);
    if (rate != null) {
      return Optional.of(rate);
    }
    val oppositeRate = data.get(to + "-" + from);
    return oppositeRate != null
        ? Optional.of(BigDecimal.ONE.divide(oppositeRate, 2, RoundingMode.HALF_UP))
        : Optional.empty();
  }
   ...
   */
  @Override
  public void upsert(@NotNull String from, @NotNull String to, @NotNull BigDecimal rate) {
    val key = from + "-" + to;
    data.put(key, rate);
  }
